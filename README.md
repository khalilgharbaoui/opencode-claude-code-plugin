...
- **A receipt, straight away**, when you asked while a turn was running: a `▌ **btw:** <your question> *sent to Claude on the side*` line in the reply you are watching, so a `/btw` typed mid-turn is visibly taken rather than looking swallowed until the answer arrives. It quotes the question back because the prompt box clears on submit and no `/btw` message is ever created, so this is the only record of what you sent. Long asides are elided here; the answer block carries the question in full. If opencode is between two streams at that moment (it was running a tool), the receipt lands when the next one opens.
