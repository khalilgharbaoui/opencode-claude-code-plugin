...
- **Inside the running turn's own reply**, as soon as the answer arrives, when you asked while Claude was working. It is written into the reply you are already watching as its own block, headed `> **btw:** <your question>`, so it renders in full markdown and stays there. The whole block, answer included, is one blockquote, so the TUI draws its quote rule down the full aside. That rule takes its colour from your theme's `markdownBlockQuote`; the plugin cannot set a colour per block, so change the theme if you want a different one. Nothing is queued and the `/btw` message itself is dropped, because the answer is already in the transcript. The turn goes on to deliver its own reply as usual.
